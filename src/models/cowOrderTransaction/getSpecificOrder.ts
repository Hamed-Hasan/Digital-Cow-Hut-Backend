...
export const getSpecificOrder: RequestHandler<{ id: string }> = async (req, res, next) => {
    try {
      ...
      const order = await OrderModel.findById(orderId).populate('cow', 'name age price location breed weight label category seller');
      if (!order) {
      ...
      if (isAuthorized) {
        responseHandler.success(res, 'Order information retrieved successfully', { order });
      } else {
